...
if (len(sys.argv)<4): 
  ...
  print ('It takes three argument');
  print ('Usage:\n\tpython quantifyAS.py ASfilePrefix SAMfiles OUTdir');
  print ('Example\n\tpython quantifyAS.py fromGTF hc.sam ./');
  sys.exit();
...
base_1 = 'Sample_1';
#base_2 = 'base_2';
dataType = 'single'; ## either single or paired
samDir = '';
input_1='.';
#input_2='.';
outDir = 'myOutput';
...
SE = sys.argv[1]+'.SE.txt';
MXE = sys.argv[1]+'.MXE.txt';
A5SS = sys.argv[1]+'.A5SS.txt';
A3SS = sys.argv[1]+'.A3SS.txt';
RI = sys.argv[1]+'.RI.txt';
AFE = sys.argv[1]+'.AFE.txt';
ALE = sys.argv[1]+'.ALE.txt';
input_1 = sys.argv[2];
outDir = sys.argv[3];
...
#
JC_seFile = open(outDir+'/JC.'+prefix+'.SE.txt', 'w');  
JC_mxeFile = open(outDir+'/JC.'+prefix+'.MXE.txt', 'w');  
JC_a5ssFile = open(outDir+'/JC.'+prefix+'.A5SS.txt', 'w');  
JC_a3ssFile = open(outDir+'/JC.'+prefix+'.A3SS.txt', 'w');  
JC_afeFile = open(outDir+'/JC.'+prefix+'.AFE.txt', 'w');  
JC_aleFile = open(outDir+'/JC.'+prefix+'.ALE.txt', 'w');  
JC_riFile = open(outDir+'/JC.'+prefix+'.RI.txt', 'w');  
#
JCEC_seFile = open(outDir+'/JCEC.'+prefix+'.SE.txt', 'w');
JCEC_mxeFile = open(outDir+'/JCEC.'+prefix+'.MXE.txt', 'w');
JCEC_a5ssFile = open(outDir+'/JCEC.'+prefix+'.A5SS.txt', 'w');
JCEC_a3ssFile = open(outDir+'/JCEC.'+prefix+'.A3SS.txt', 'w');
JCEC_afeFile = open(outDir+'/JCEC.'+prefix+'.AFE.txt', 'w');
JCEC_aleFile = open(outDir+'/JCEC.'+prefix+'.ALE.txt', 'w');
JCEC_riFile = open(outDir+'/JCEC.'+prefix+'.RI.txt', 'w');
#
...
#  f3.write(h3+'\n');
  ...
  for k in sorted(sup.keys()): 
    ...
    f1.write(str(k)+'\t'+','.join(map(str,cnt[k][CT1][S1][I]))+'\t'+','.join(map(str,cnt[k][CT1][S1][S]))+'\n'); ## need to correct this to handle replicates...
...
CT1_header = 'ID\tIJC_'+'\tSJC_'+'';
CT2_header = 'ID\tIC_'+'\tSC_'+'';
CT3_header = 'ID\tIC_'+'\tSC_'+'';
#
## SE ##
...
writeInputFile(CT1_header,CT2_header,CT3_header,JC_afeFile,JCEC_afeFile,c_afe,s_afe);
### ALE ##...
#writeInputFile(CT1_header,CT2_header,CT3_header,JC_aleFile,JCEC_aleFile,JCECPE_aleFile,c_ale,s_ale);
writeInputFile(CT1_header,CT2_header,CT3_header,JC_aleFile,JCEC_aleFile,c_ale,s_ale);
## RI ##...
